...
#define FIELD_WIDTH                                                             \
	(SCREEN_WIDTH / PLAYER_SIZE) /* Cantidad de posiciones por fila del tablero \
								  */
#define FIELD_HEIGHT \
	(SCREEN_HEIGHT / \
	 PLAYER_SIZE) /* Cantidad de posiciones por columna del tablero */
#define FIELD_WIDTH_POSITIONS \
	(FIELD_WIDTH / 8 + 1) /* Cantidad de bytes que ocupa una fila del tablero */

#define INVALID_QTY_PLAYERS                                                      \
	"Actualmente el juego solo soporta el modo individual o el de 2 jugadores, " \
	"vuelva pronto!\n"
...
#define ZEN_KEYS                                                                 \
	"El jugador se mueve con W A S D o con las flechas\ny activa turbo con Z o " \
	"/\n"

#define START_GAME \
	"Presione espacio para comenzar, y ESC para salir en el juego\n"
#define GAME_OVER "Fin del Juego!!!\n"
#define PLAY_AGAIN \
	"Desea volver a jugar? (espacio para confirmar, esc para salir)"
...
/**
 * @brief  Actualiza la posicion del jugador, chequea colisiones y actualiza el
 * tablero
 * @param  p: Puntero al jugador
 * @return 1 si el jugador colisiono, 0 si no
 */
 ...
 */
static void setDirections(Player *p, uint8_t key, uint8_t up, uint8_t left,
						  uint8_t down, uint8_t right, uint8_t turbo);
...
uint8_t _field[FIELD_WIDTH_POSITIONS]
			  [FIELD_HEIGHT]; /* Tablero con optimizacion en bits */
...
static void tron() {
	...
	do {
		...
		while (_playing) {
			...
			setDirections((_qtyPlayers == 2) ? &_p2 : &_p1, key, KEY_UP, KEY_LEFT,
						  KEY_DOWN, KEY_RIGHT, KEY_SLASH);
			if (nextTicks - ticks >=
				DELTA_TICK) { /* Si transcurrio un game tick actualiza el estado */
				ticks = nextTicks;
				if (_qtyPlayers == 2)
					loop(_field);
				else
	...
	for (int i = 0; i < FIELD_WIDTH; i++)
		for (int j = 0; j < FIELD_HEIGHT; j++)
			if (_field[i / 8][j] & (1 << (i % 8)))
				drawRect(i * PLAYER_SIZE, j * PLAYER_SIZE, PLAYER_SIZE, PLAYER_SIZE,
						 RED);
}
#endif
...
static void initializeGame() {
	...
	_playing = 1;
	_p1 = (Player){FIELD_WIDTH * STARTING_OFFSET_1,
				   FIELD_HEIGHT / 2,
				   1,
				   0,
				   1,
				   ...
				   0,
				   _p1.wins};
	_p2 = (Player){FIELD_WIDTH * STARTING_OFFSET_2 - 1,
				   FIELD_HEIGHT / 2,
				   -1,
				   0,
				   -1,
...
static void setDirections(Player *p, uint8_t key, uint8_t up, uint8_t left,
	...
	if (key == up && p->oldVy == 0) {
		...
		p->vy = 0;
			 ...
			 ticks - p->lastTurbo >=
				 TURBO_COOLDOWN + TURBO_TICKS) { // Si ya paso el cooldown y se
												 // toca se activa
		p->lastTurbo = ticks;
	}
}
...
static void loop() {
	drawRect(_p1.x * PLAYER_SIZE, _p1.y * PLAYER_SIZE, PLAYER_SIZE, PLAYER_SIZE,
			 _p1.trailColor);
	drawRect(_p2.x * PLAYER_SIZE, _p2.y * PLAYER_SIZE, PLAYER_SIZE, PLAYER_SIZE,
			 _p2.trailColor);
	int col1 = update(&_p1);
	int col2 = update(&_p2);
...
static int update(Player *p) {
	...
	for (int i = 0; i < realV && !collided; i++) {
		...
		drawRect(p->x * PLAYER_SIZE, p->y * PLAYER_SIZE, PLAYER_SIZE, PLAYER_SIZE,
				 p->color);
	}
	return collided;
}